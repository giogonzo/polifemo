...
function createExpressRecorder(target, fixtureDir) {
  ...
  function saveConfig(prettyPrint) {
  ...
  function saveScreenshot(name, image) {
    var imagePath = path.join(fixtureDir, 'images', name + '.txt');
  ...
  app.post(SAVE_SCREENSHOT_ROUTE, function (req, res) {
    ...
    saveScreenshot(imageCounter, req.body.image);
